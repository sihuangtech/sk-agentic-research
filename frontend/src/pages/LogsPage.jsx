...
export default function LogsPage() {
  ...
  const actions = <>
    <button className="action-secondary" onClick={() => setFollowing((value) => !value)}>{following ? <Pause size={15} /> : <Play size={15} />}{following ? t('logs.pause') : t('logs.resume')}</button>
    <button className="action-danger" onClick={() => setLogs([])}><Trash2 size={15} />{t('logs.clear')}</button>
  </>;
  return (
    <div className="mx-auto flex min-h-[80vh] max-w-7xl flex-col">
      <PageHeader eyebrow={t('logs.eyebrow')} title={t('logs.title')} description={t('logs.description')} actions={actions} />
      <div className="mb-3 flex gap-2">
        {['all', 'info', 'warning', 'error'].map((name) => <button key={name} onClick={() => setFilter(name)} className={`rounded-lg px-3 py-2 text-xs font-bold ${filter === name ? 'bg-cyan-300 text-slate-950' : 'bg-white/5 text-slate-400'}`}>{t(`logs.filters.${name}`)}</button>)}
      </div>
      <div ref={container} className="terminal-scrollbar flex-1 overflow-y-auto rounded-2xl border border-white/10 bg-black/40 p-5 font-mono text-xs leading-6">
        ...
        {!visible.length && <p className="text-slate-600">{t('logs.empty')}</p>}
      </div>
    </div>
  );
}
